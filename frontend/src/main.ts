...
// Enhanced method to handle different types of remote game disconnections
private handleRemoteGameDisconnection(reason: string, code?: number): void {
    console.log(`Handling remote game disconnection: ${reason} (code: ${code})`);
    
    if (this.onlineGameState.gameSocket && !this.onlineGameState.gameFinished) {
        // Send graceful disconnect message if possible
        try {
            if (this.onlineGameState.gameSocket.readyState === WebSocket.OPEN) {
                ...
                this.onlineGameState.gameSocket.send(JSON.stringify(disconnectMessage));
            }
        } catch (error) {
        ...
        if (this.onlineGameState.gameSocket.readyState !== WebSocket.CLOSED) {
            this.onlineGameState.gameSocket.close(code || 1001, reason);
        }
    }
    
    // Update UI
    this.updateRemoteGameStatus('Disconnected', `Left game: ${reason}`);
    
    // Mark game as finished locally
